...
app = firebase_admin.initialize_app(
    options={"projectId": config["global"]["project_id"]}
)
db = firestore.client()
...
def create_campaign(user_id, campaign: Campaign):
    ...
    update_time, campaign_ref = user_ref.collection("campaigns").add(
        json.loads(json.dumps(campaign.__dict__, default=to_serializable))
    )
...
def list_campaigns(user_id):
    campaign_col = (
        db.collection("users").document(user_id).collection("campaigns").stream()
    )
    list_campaigns = []
