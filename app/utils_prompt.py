...
def render_email_prompt_design(
        project_id: str,
        location: str,
        state_key: str,
    ...
    Renders a design for:
        - Prompt engineering for email and blog post generation
        - Translation
            ...
            try:
                ...
                with st.spinner('Generating response ...'):
                    ...
                    response = llm.predict(
                            prompt=EMAIL_PROMPT_TEMPLATE.format(theme=custom_prompt),
    ...
    if show_response and state_key in st.session_state:
        st.write('**Response**')
        email = st.session_state[state_key]
        email = email.replace('Subject:', '**Subject:**')
        email = email.replace('Body:', '**Body:**')
        st.write(email)
        ...
        if translate:
            ...
            if translate_submit_button:
                with st.spinner("Translating..."):
                    st.session_state[
                        f"{state_key}_translated"] = translate_client.translate(
            ...
            if f"{state_key}_translated" in st.session_state:
                ...
                st.write(st.session_state[f"{state_key}_translated"])
